...
# output file
#SBATCH --output slurm/slurm_data_generation.out
...
data_gen_n_samples_per_sim=2000 #200000  # How many simulated trials per call to the simulator ?
...
jobID_1=$(sbatch -p batch --array=0-150 sbatch_data_generation.sh --config_file $data_generation_config_file | cut -f 4 -d' ')

jobID_2=$(sbatch -p batch --account=carney-frankmj-condo --array=0-150 sbatch_data_generation.sh --config_file $data_generation_config_file | cut -f 4 -d' ')
